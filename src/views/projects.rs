...
#[component]
pub fn ProjectsViewOld() -> Element {
    use_effect(|| {
    ...
    let cards = rsx! {
        if !projects.is_empty() {
            for (name , project) in projects().iter() {
                ProjectCardOld { name, project: project.clone(), delete_ctx }
            }
        } else {
...
#[component]
pub fn ProjectCardOld(name: String, project: Project, delete_ctx: Store<DeleteCtx>) -> Element {
...
#[component]
fn ProjectCard(name: String, project: Project, delete_ctx: Store<DeleteCtx>) -> Element {
    let name_clone = name.clone();
